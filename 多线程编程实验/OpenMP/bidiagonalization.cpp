...
//
// OpenMP 优化点：
//   1. Householder 变换中各行相互独立的 B/U/V 更新并行执行；
//   2. 将可独立执行的 dot + AXPY 融合到同一行任务中，减少中间数组访问；
//   3. 小规模尾部问题保持串行，避免 OpenMP 调度开销超过计算收益。
...
namespace
{
    constexpr long long MIN_OMP_ROW_WORK = 65536;
    ...
    static int parse_positive_env(const char *name)
    {
        ...
        return static_cast<int>(std::min<long>(parsed, MAX_OMP_WORKERS));
    }

    static int detect_omp_thread_count()
    {
        const int env_threads = parse_positive_env("OMP_NUM_THREADS");
        if (env_threads > 0)
        {
            return env_threads;
        }
        ...
        if (nodefile != nullptr && nodefile[0] != '\0')
        {
            std::ifstream input(nodefile);
            std::string line;
            int slots = 0;
            ...
            if (slots > 0)
            {
                return std::min(slots, MAX_OMP_WORKERS);
            }
        }

        return std::min(omp_get_max_threads(), MAX_OMP_WORKERS);
    }
}
...
Matrix to_bidiagonal(const Matrix &A, Matrix &U, Matrix &V)
{
    ...
    omp_set_dynamic(0);
    omp_set_num_threads(detect_omp_thread_count());
    ...
    for (int k = 0; k < n; ++k)
    {
        ...
        if (norm_x > 1e-14 && k < m - 1)
        {
            ...
            if (vTv > 1e-28)
            {
                ...
                // 更新 B(k:m, k:n)：
                //   B_row -= beta * v[i] * w
#pragma omp parallel for schedule(static) if (static_cast<long long>(m - k) * (n - k) >= MIN_OMP_ROW_WORK)
                for (int i = 0; i < m - k; ++i)
                {
                ...
                // 每一行 U(i, k:m) 是连续内存，
                // 因此内积和向量更新都可以用 SIMD。
#pragma omp parallel for schedule(static) if (static_cast<long long>(m) * (m - k) >= MIN_OMP_ROW_WORK)
                for (int i = 0; i < m; ++i)
                {
                    double *urow = &U.at(i, k);
                    const double coeff = dot_product_simd(urow, v.data(), m - k);
                    const double scale = -beta * coeff;
...
#pragma omp parallel for schedule(static) if (static_cast<long long>(m - k) * (n - k - 1) >= MIN_OMP_ROW_WORK)
                    for (int i = 0; i < m - k; ++i)
                    {
                        double *brow = &B.at(k + i, k + 1);
                        const double coeff = dot_product_simd(brow, v.data(), n - k - 1);
                        const double scale = -beta * coeff;
...
#pragma omp parallel for schedule(static) if (static_cast<long long>(n) * (n - k - 1) >= MIN_OMP_ROW_WORK)
                    for (int i = 0; i < n; ++i)
                    {
                        double *vrow = &V.at(i, k + 1);
                        const double coeff = dot_product_simd(vrow, v.data(), n - k - 1);
                        const double scale = -beta * coeff;
