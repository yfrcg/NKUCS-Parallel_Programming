...
if ! g++ main.cpp gkh.cpp bidiagonalization.cpp -o main "$OPT_FLAG" -fopenmp -lpthread -std=c++17; then
    echo "OpenMP 编译失败"
    exit 1
fi

QSUB_VARS="SVD_SEED=${SEED},SVD_WORKDIR=${SCRIPT_DIR},OMP_NUM_THREADS=${CORES}"
if [ -n "${SVD_GKH_STATS:-}" ]; then
    QSUB_VARS="${QSUB_VARS},SVD_GKH_STATS=${SVD_GKH_STATS}"
fi
if [ -n "${SVD_GKH_SCHED:-}" ]; then
    QSUB_VARS="${QSUB_VARS},SVD_GKH_SCHED=${SVD_GKH_SCHED}"
fi

if ! jobid=$(qsub -v "$QSUB_VARS" \
    -o "${RESULT_PATH}test.o" -e "${RESULT_PATH}test.e" \
    -l nodes="${NODES}:ppn=${CORES}" qsub.sh); then
    echo "PBS 作业提交失败"
    exit 1
fi
