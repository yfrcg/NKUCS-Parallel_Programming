...
本目录的实现包含两类 OpenMP 并行：

1. `bidiagonalization.cpp` 对 Householder 变换中互相独立的 `B`、`U`、`V` 行更新使用 `parallel for`。仅当剩余工作量超过阈值时启用并行，避免尾部小问题的线程调度开销。
2. `gkh.cpp` 将互不冲突的活动块组成批次，并在批次内部使用 OpenMP 动态调度执行 `one_block_step`。相邻活动块由于会写入 `B` 的边界窗口，被安排到不同批次以避免数据竞争。

线程数优先读取 `OMP_NUM_THREADS`；在 PBS 作业中若未显式设置，则读取 `PBS_NODEFILE` 中分配的 slot 数。

正式正确性和性能测试仍可使用框架脚本，例如：

```bash
bash test.sh 2 1 4 -O O2 -s 20260410
```

如果需要手动固定 OpenMP 绑定策略，可以直接提交：

```bash
g++ main.cpp gkh.cpp bidiagonalization.cpp -o main -O2 -fopenmp -lpthread -std=c++17
qsub -v SVD_SEED=20260410,OMP_NUM_THREADS=4,OMP_PROC_BIND=close,OMP_PLACES=cores -l nodes=1:ppn=4 qsub.sh
```

### GKH 负载测试

原始随机样例可能始终只有一个活动块，此时 GKH 的 block 间并行不会触发。`gkh_load_balance.cpp` 提供了独立的多活动块输入，用于比较静态和动态调度的线程负载。

```bash
g++ gkh_load_balance.cpp gkh.cpp -o main -O2 -fopenmp -lpthread -std=c++17
qsub -v SVD_GKH_STATS=1,SVD_GKH_SCHED=static,OMP_NUM_THREADS=4 -l nodes=1:ppn=4 qsub.sh
qsub -v SVD_GKH_STATS=1,OMP_NUM_THREADS=4 -l nodes=1:ppn=4 qsub.sh
```

完成负载测试后，重新编译 `main.cpp` 版本再执行正式测试。
